...
Ext.apply(Ext.form.VTypes, {
	...
	"repassword" : function(_v, field) {
		if (field.confirmTO) { // form表单的repassword 要有一个属性confirmTO:xxxx
								// 对应password的id
	...
	"postcodeText" : "该输入项目必须是邮政编码格式，例如：310000",
	"postcodeMask" : /[0-9]/i,
	...
	/**
	 * 格式例子 items: [{ fieldLabel: 'Start Date', name: 'startdt', id: 'startdt',
	 * vtype: 'daterange', endDateField: 'enddt' //对应后一个日期的id },{ fieldLabel:
	 * 'End Date', name: 'enddt', id: 'enddt', vtype: 'daterange',
	 * startDateField: 'startdt' //对应前一个日期的id }]
	 * 
	 */

	// 文件的验证 jpg gif
	imageV : function(_v, field) {
	},

	// excel 文件
	"excel" : function(_v) {
		var exc = _v.toLowerCase();
		if (exc.indexOf('.xls')!=-1) {
				return true;
		} else
			return false;
	},
	'excelText' : '选择的文件类型错误！！文件类型格式例如: .xls'
	//'ageMask' : /[0-9]/i
